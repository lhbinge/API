...
xt <- xtable(summaryfunction(artdata$hammer_price), caption="Descriptive statistics of auction hammer prices", xtable.caption.placement = 'bottom')
...
```{r figure4, echo=FALSE, cache = TRUE, fig.height=3.5, fig.width=7.5, fig.cap="Hedonic South African art price indices (2000Q1=100)"}
...
##Hedonic Extensions
###Stratified hedonic regressions[^19]

[^19]: Another approach, suggested by Eurostat (2013), is to use hedonic regression at the stratum level to adjust for quality mix changes. This two-stage approach combines hedonics estimates at the lower stratum level and explicit weighting at the upper level to form an overall price index. This approach has the advantage of examining different price indices for different market segments, as it is possible that the different segments could exhibit different price trends. This process was followed by estimating hedonic models for each of the different mediums separately. The indices were then combined by weighting them by their corresponding sales value shares. Two stratified hedonic indices were estimated and could be included if necessary: the weighted average of the medium indices and the Fisher ideal price index. The results indicate that the stratified hedonic indices follow a similar cyclical pattern, but at a higher level that the full pooled sample index.

###Hedonic Quantile regressions[^20]

[^20]: The South African art market may be segmented for a number of reasons. As art is indivisible, small investors are generally not able to invest in more expensive works. Wealthy individuals may be less tempted to buy at the lower end of the market, where works do not signal the same social status. The more expensive parts of the market may be more prone to speculation. The distribution of returns may therefore be skewed (Renneboog & Spaenjers, 2012). Quantile regressions can be useful in such a case. Although OLS regressions provide estimates for the conditional means, quantile regressions can characterise the entire distribution of the dependent variable. The characteristic prices and the changes in price levels are allowed to vary across the distribution of auction prices. 
Renneboog & Spaenjers (2012) used quantile regressions to show that historical rates of appreciation varied across the price distribution. They found larger average price appreciations and higher volatilities in the more expensive price brackets. This implied that masterpieces did not underperform the rest of the market over their sample period, in contrast to findings by Pesando (1993) and Mei & Moses (2002). High-end art appreciated faster during boom periods and decreased more during downturns. 
Fedderke & Li (2014) suggested that the South African art market should be segmented into three price ranges and found different relationships for the three market segments. Quantile regressions were used to investigate this possibility, by estimating the hedonic models on the 75th, 50th, and 25th percentiles. The regression results indicated that a few of the variables had different impacts in different parts of the price distribution. For instance, the size of the artwork was more important in the higher quantiles. The quantile hedonic price indices were then constructed by chain-linking the coefficients of the dummies for the relevant quantiles. We could illustrate the resulting quantile art price indices. The lower end of the market seems to have depreciated less after the peak in 2008. This could indicate that some people are being priced out of the higher end of the art market, increasing the lower end prices more than the higher end prices (Els & Von Fintel, 2014).
...
```{r figure5, echo=FALSE, cache = TRUE, fig.height=4, fig.width=7.5, fig.cap="Pseudo repeat sales South African art price indices (2000Q1=100)"}
...
Out-of-sample prediction tests offer another perspective on the reliability and robustness of the index estimation methodologies. To conduct this test, the sample is randomly divided into two sub-samples , and the index is estimated based on each of the sub-samples separately. The resulting indices can then be compared . The two indices in any pair are almost the same (no visually apparent differences) with high cross-correlation coefficients in the returns. Furthermore, we conduct mean-comparison tests between the two indices in each subsample pair with the null hypothesis as: 

H0: mean(Index1,t - Index2,t)=0; where t indicates the period number. 

The H0 could not be rejected, implying that the two indices based on two randomly constructed sub-samples are statistically indistinguishable from each other [@Guo2014]. 
Another option for evaluation would be to examine the predictive ability of the models. @Kuethe2011 suggested evaluating the ability of a range of aggregate house price indices to predict house prices out-of-sample at the transaction level. @Nagaraja2014 compared the performance of their autoregressive index to four traditional repeat sales indices in terms of predictive ability. 

In order to evaluate the indices, the transactions can be divided into two subsets: the training and the testing sets. These subsets can be separated either in time series (e.g. after a cut-off date) or randomly (e.g. the uniform distribution). The training set is used to estimate each of the index methodologies and the estimated coefficients are then used to create the predicted transaction prices in the testing set. Performance measures (such as the MAE and RMSE) can be used to examine the difference between the observed and predicted transaction prices, based on the coefficient estimates of the training set. This provides information on how well the models do when applied at the micro level. Predictions also provide a better quantitative measure of the effectiveness of the index in describing market trends. Table 6 reports the results for this metric. The ps-RS methods have RMSE values... 
